...
def train(cfg, local_rank, distributed):
    ...
    if distributed:
        model = torch.nn.parallel.DistributedDataParallel(
            ...
            find_unused_parameters=True  # 添加此参数以处理未使用参数
        )
        # model._set_static_graph()  # 设置静态图
    ...
    if cfg.MODEL.DOMAIN_ADAPTATION_ON:
        ...
        if cfg.MODEL.MIC_ON:
            ...
            # 从主模型加载参数（确保主模型已经是DDP）
            model_t.load_state_dict(model.module.state_dict() if distributed else model.state_dict())
